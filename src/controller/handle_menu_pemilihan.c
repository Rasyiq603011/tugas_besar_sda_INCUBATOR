...
int handle_pemilihan_kursi(address studio, User* user)
{
    ...
    Riwayat* new_riwayat = create_riwayat(get_judul_film(current_jadwal->First->info.jadwal), date_to_string(get_tanggal_tayang(jadwal_dipilih)), time_to_string(get_waktu_start(jadwal_dipilih)), get_harga_tiket(jadwal_dipilih), info_kursi);
